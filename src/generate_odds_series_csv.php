...
$db = new PDO("mysql:host=localhost;dbname=odds_series;charset=utf8", 'root', 'toor');
...
$query = "SELECT `date`,ID, result from 
				 ...
				 AND `date` < '2016-03-01' AND 
				 result not like '%CAN%' AND result not like '%WO%'
...
while($row = $r->fetch(PDO::FETCH_OBJ)) 
{
	...
	if ($odds_exist) {
		...
		// save matrix to file
		file_put_contents("..\\data\\odds_series\\match_".$row->ID.'_'.date('Y_m_d_H_i_s',strtotime($row->date)).'_'.$score1.'_'.$score2.'.txt',$matrix_str);  
	}
    $current_count++;
}
